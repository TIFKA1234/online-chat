...
class MainWindow(CTk):
    def __init__(self):
        ...
        # Меню (серый фон)
        self.label = None
        self.menu_frame = CTkFrame(self, width=30, height=300, fg_color="#404040")
        ...
        # Основное поле чату (прозрачное)
        self.chat_field = CTkScrollableFrame(self, fg_color="transparent")
        self.chat_field.place(x=0, y=0)

        # Поле введення та кнопки
        self.message_entry = CTkEntry(self, placeholder_text='Введіть повідомлення:', height=40)
        self.message_entry.place(x=0, y=0)
        self.message_entry.bind("<Return>", lambda event: self.send_message())  # ⬅️ Enter для відправки
    ...
    def toggle_show_menu(self):
        if self.is_show_menu:
            ...
            self.label = CTkLabel(self.menu_frame, text='Імʼя', fg_color="#c0c0c0")
            self.label.pack(pady=40)
            self.entry = CTkEntry(self.menu_frame, placeholder_text="Ваш нік...", fg_color="#e0e0e0")
            self.entry.pack()
            # Кнопка збереження
            self.save_button = CTkButton(self.menu_frame, text="Зберегти", fg_color="blue", command=self.save_name)
